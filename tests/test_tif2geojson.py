...
class TestTif2geojson(unittest.TestCase):
    ...
    def test_entries_have_description_in_specified_lowercase_language(self):
        geojson = tif2geojson(self.fullsample, lang='fr')
        properties = geojson['features'][0]['properties']
        description = u"Chambres confortables, service buanderie, piscine, tennis, garage fermé, grand parking, Wifi.<br/>"
        self.assertEqual(properties['description'], description)
    ...
    def test_entries_have_phone(self):
        geojson = tif2geojson(self.fullsample)
        properties = geojson['features'][0]['properties']
        self.assertEqual(properties['phone'], u'+330565297390')
